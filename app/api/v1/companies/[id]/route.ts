 ...
 */
export const GET = withApiAuth(async (request: NextRequest, { apiKey }, routeContext?: { params?: Promise<{ id: string }> }) => {
  ...
  // Build query
  let query = supabase.from('companies').select('*').eq('id', id).single()
 ...
 */
export const PUT = withApiAuth(async (request: NextRequest, { apiKey }, routeContext?: { params?: Promise<{ id: string }> }) => {
  // Check scopes
