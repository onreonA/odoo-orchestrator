import { createClient } from '@/lib/supabase/server'
import { Button } from '@/components/ui/button'
import { Plus, Users } from 'lucide-react'
import Link from 'next/link'

export default async function DepartmentsPage() {
  const supabase = await createClient()

  // Get user profile to get company_id
  const {
    data: { user },
  } = await supabase.auth.getUser()

  if (!user) {
    return <div>Kullanıcı bulunamadı</div>
  }

  const { data: profile } = await supabase.from('profiles').select('*').eq('id', user.id).single()

  const companyId = profile?.company_id

  // Get departments
  const { data: departments, error } = companyId
    ? await supabase
        .from('departments')
        .select('*')
        .eq('company_id', companyId)
        .order('created_at', { ascending: false })
    : { data: null, error: null }

  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Departmanlar</h1>
          <p className="text-[var(--neutral-600)] mt-1">Firma departmanlarını yönetin</p>
        </div>
        <Link href="/departments/new">
          <Button size="lg">
            <Plus className="w-4 h-4 mr-2" />
            Yeni Departman Ekle
          </Button>
        </Link>
      </div>

      {/* Departments List */}
      {error ? (
        <div className="bg-[var(--error)]/10 border border-[var(--error)] rounded-lg p-4 text-[var(--error)]">
          Hata: {error.message}
        </div>
      ) : !companyId ? (
        <div className="bg-white rounded-xl p-12 border border-[var(--neutral-200)] text-center">
          <Users className="w-16 h-16 text-[var(--neutral-300)] mx-auto mb-4" />
          <h3 className="text-xl font-semibold mb-2">Firma seçilmedi</h3>
          <p className="text-[var(--neutral-600)] mb-6">
            Departman oluşturmak için önce bir firma seçmelisiniz
          </p>
        </div>
      ) : departments && departments.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {departments.map((department: any) => (
            <Link
              key={department.id}
              href={`/departments/${department.id}`}
              className="bg-white rounded-xl p-6 border border-[var(--neutral-200)] hover:shadow-lg transition-all hover:-translate-y-1"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="p-3 rounded-lg bg-[var(--brand-primary-50)]">
                  <Users className="w-6 h-6 text-[var(--brand-primary-500)]" />
                </div>
                <span className="text-xs px-2 py-1 rounded-full bg-[var(--neutral-100)] text-[var(--neutral-600)]">
                  {department.status || 'active'}
                </span>
              </div>
              <h3 className="text-xl font-semibold mb-2">{department.name}</h3>
              <p className="text-sm text-[var(--neutral-600)] mb-4">
                {department.description || 'Açıklama yok'}
              </p>
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-xs text-[var(--neutral-500)]">Teknik İsim</div>
                  <div className="text-sm font-medium">{department.technical_name || '-'}</div>
                </div>
                <div className="text-sm text-[var(--neutral-500)]">
                  {new Date(department.created_at).toLocaleDateString('tr-TR')}
                </div>
              </div>
            </Link>
          ))}
        </div>
      ) : (
        <div className="bg-white rounded-xl p-12 border border-[var(--neutral-200)] text-center">
          <Users className="w-16 h-16 text-[var(--neutral-300)] mx-auto mb-4" />
          <h3 className="text-xl font-semibold mb-2">Henüz departman yok</h3>
          <p className="text-[var(--neutral-600)] mb-6">İlk departmanınızı ekleyerek başlayın</p>
          <Link href="/departments/new">
            <Button>
              <Plus className="w-4 h-4 mr-2" />
              Yeni Departman Ekle
            </Button>
          </Link>
        </div>
      )}
    </div>
  )
}
