...
test.describe('Excel Import Module', () => {
  ...
  test('should show Odoo connection fields', async ({ page }) => {
    ...
    const dbInput = page.locator('input[placeholder*="database"], input[placeholder*="veritabanı"], input[placeholder*="odoo_db"]')
    await expect(dbInput.first()).toBeVisible()
    
    // Check for username field
    const usernameInput = page.locator('input[placeholder*="admin"], input[placeholder*="kullanıcı"]')
